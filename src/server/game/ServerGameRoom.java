...
public class ServerGameRoom extends DGame implements Runnable {
	...
	public ServerGameRoom(String nameOfServer, String gamePassword, int numberOfBots, Player player) {
		...
		if (numberOfBots <= 3) {
			for (int i = 1; i <= numberOfBots; i++) {
				this.listOfPlayers.addLast(new AIServer("Robot" + i, this));// Insertion
																			// of
																			// AI
				this.listOfPlayers.getLast().setItARobot(true);
			}
	...
	public void run() {
		...
		while (true) {
			this.restartPlayersSettings();// restarts possisonTwoOfClubs and
											// areCardsDown
			...
			while (true) {
				if (listOfPlayers.get(0).isAreCardsDropped() == true && listOfPlayers.get(1).isAreCardsDropped() == true
