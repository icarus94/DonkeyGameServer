...
public class ClientServerConnection implements Runnable {
	...
	public void run() {
		try {
			 ...
			 * System.out.println(socketForConnection.getRemoteSocketAddress().
			 * toString());
			 */
			...
			if (existingGameForThisPlayer != null) {
				objectClientOutput.writeObject(new String("Do you want to reconnect to:"
						+ existingGameForThisPlayer.getName() + "as" + checkForPlayerNameInUnfinishedGame()));
				String input = null;
				while (input == null) {
			...
			while (true) {
				...
				objectClientOutput.writeUnshared(ServerControl.listOfGameRoomsTypeDGame());// sending
																							// list
																							// of
																							// GameRooms
																							// instanceof
				...
				mainLoop: switch (inputStringFromClient) {
				case "refresh":
					break;
				case "quickgame": {
				...
				case "newGameRoom": {
					...
					int numberOfBotsTypeInt = Integer.parseInt(numberOfBots); // Parsing
																				// to
																				// INT
				...
				case "connectToGameRoom": {
					...
					if (!serverReady) {
						...
						System.out
								.println("server done playing or full or inccorect password for client " + playerName);
						break mainLoop;
					}
		...
		for (int i = 0; i < ServerControl.listOfGameRooms.size(); i++) {
			for (int j = 0; j < ServerControl.listOfGameRooms.get(i).getListOfPlayers().size(); j++) {
				if (ServerControl.listOfGameRooms.get(i).getListOfPlayers().get(j) != null
						&& ServerControl.listOfGameRooms.get(i).getListOfPlayers().get(j)
								.getSocketAddressOfPlayer() != null
						&& ServerControl.listOfGameRooms.get(i).getListOfPlayers().get(j).getSocketAddressOfPlayer()
								.equals(socketForConnection.getRemoteSocketAddress())
						&& ServerControl.listOfGameRooms.get(i).getListOfPlayers().get(j)
								.getPortOfPlayer() == socketForConnection.getPort()) {
